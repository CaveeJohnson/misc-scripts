...
minmod.ESPData = {
	...
	Name		= function(e) return ((e.Nick and e:Nick()) or e.PrintName or e:GetClass()) .. " (" .. (e.Health and e:Health() or "100") .. " HP)" end,
...
function minmod:BHop(cmd)
	...
	if not self:GetVar("Enable Auto-Jump") or minmod.ply:GetMoveType() == MOVETYPE_NOCLIP then
...
local function vl(tg,ep,oc)
	return not(not tg or not IsValid(tg)or util.TraceLine({mask=MASK_SHOT,endpos=oc,start=ep,filter={me,tg}}).Fraction~=1 or not tg:Alive()or tg:IsDormant()or tg==me)
end

function minmod:AimBot(cmd)

    if not self:GetVar("Enable Aimbot") then
	...
	if sh then
		cmd:SetButtons(a(cmd:GetButtons(), n(IN_ATTACK)))
	end
	sh=not sh
	...
	if tg then
		local ea=tg:LookupAttachment("eyes")
		local tp=((ea and tg:GetAttachment(ea)and tg:GetAttachment(ea).Pos)or oc)-me:GetVelocity()*ti
		local va=(tp-ep):Angle()
...
hook.Add("CreateMove", "\0minmod_aimbot", function(cmd) minmod:AimBot(cmd) end)

minmod:SetVar("Enable Triggerbot", false, "Arrow Up")

local LastTrigger = CurTime()
local UnAttack = math.huge

function minmod:TriggerBot()

    if not self:GetVar("Enable Triggerbot") then
        ...
        local trace = minmod.ply:GetEyeTrace().Entity

        if trace and trace:IsPlayer() and LastTrigger < CurTime() and not self:IsFriend(trace) then
            minmod.ply:ConCommand( "+attack" )
            LastTrigger = CurTime() + 0.5
            UnAttack = CurTime() + 0.1
        end
